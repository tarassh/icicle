...
public:
  ...
  // return modulus
    ...
    add_sub_limbs_device(device const ff_storage& xs, device const ff_storage& ys, device ff_storage& rs)
    {
        ...
        r[0] = SUBTRACT ? ptx::sub_cc(x[0], y[0], carry_flag) : ptx::add_cc(x[0], y[0], carry_flag);
        for (unsigned i = 1; i < (CARRY_OUT ? TLC : TLC - 1); i++)
            r[i] = SUBTRACT ? ptx::subc_cc(x[i], y[i], carry_flag) : ptx::addc_cc(x[i], y[i], carry_flag);
        if (!CARRY_OUT) {
            r[TLC - 1] =
            SUBTRACT ? ptx::subc(x[TLC - 1], y[TLC - 1], carry_flag) : ptx::addc(x[TLC - 1], y[TLC - 1], carry_flag);
            return 0;
        }
        return SUBTRACT ? ptx::subc(0, 0, carry_flag) : ptx::addc(0, 0, carry_flag);
    }
    ...
    add_sub_limbs_device(device const ff_wide_storage& xs, device const ff_wide_storage& ys, device ff_wide_storage& rs)
    {
        ...
        for (unsigned i = 1; i < (CARRY_OUT ? 2 * TLC : 2 * TLC - 1); i++)
            r[i] = SUBTRACT ? ptx::subc_cc(x[i], y[i], carry_flag) : ptx::addc_cc(x[i], y[i], carry_flag);
        if (!CARRY_OUT) {
            r[2 * TLC - 1] = SUBTRACT ? ptx::subc(x[2 * TLC - 1], y[2 * TLC - 1], carry_flag)
                                    : ptx::addc(x[2 * TLC - 1], y[2 * TLC - 1], carry_flag);
            return 0;
        }
        return SUBTRACT ? ptx::subc(0, 0, carry_flag) : ptx::addc(0, 0, carry_flag);
    }
    ...
    template <bool SUBTRACT, bool CARRY_OUT>
    static constexpr inline uint32_t
    add_sub_limbs_host(device const ff_storage& xs, device const ff_storage& ys, device ff_storage& rs)
    {
        device const uint32_t* x = xs.limbs;
        device const uint32_t* y = ys.limbs;
        device uint32_t* r = rs.limbs;
    ...
    static inline void
    mul_n_msb(device uint32_t* acc, device const uint32_t* a, uint32_t bi, size_t n = TLC, size_t start_i = 0)
    {
    ...
    static inline void cmad_n(device uint32_t* acc, device const uint32_t* a, uint32_t bi, size_t n = TLC)
    {
        ...
        // acc = acc + bi*A[::2]
        uint32_t carry = 0;
        acc[0] = ptx::mad_lo_cc(a[0], bi, acc[0], carry);
        acc[1] = ptx::madc_hi_cc(a[0], bi, acc[1], carry);
        for (size_t i = 2; i < n; i += 2) {
            acc[i] = ptx::madc_lo_cc(a[i], bi, acc[i], carry);
            acc[i + 1] = ptx::madc_hi_cc(a[i], bi, acc[i + 1], carry);
        }
    }
    ...
    cmad_n_msb(device uint32_t* acc, device const uint32_t* a, uint32_t bi, size_t n = TLC, size_t a_start_idx = 0)
    {
        ...
        // acc = acc + bi*A[::2]
        uint32_t carry = 0;
        acc[a_start_idx] = ptx::mad_lo_cc(a[a_start_idx], bi, acc[a_start_idx], carry);
        acc[a_start_idx + 1] = ptx::madc_hi_cc(a[a_start_idx], bi, acc[a_start_idx + 1], carry);
#pragma unroll
    ...
    static inline void
    mad_row(device uint32_t* odd, device uint32_t* even, device const uint32_t* a, uint32_t bi, size_t n = TLC)
    {
        // odd = odd + bi*A
        // even = even + bi*A
        uint32_t carry = 0;
        cmad_n(odd, a + 1, bi, n - 2);
        odd[n - 2] = ptx::madc_lo_cc(a[n - 1], bi, 0, carry);
        odd[n - 1] = ptx::madc_hi(a[n - 1], bi, 0, carry);
        cmad_n(even, a, bi, n);
        odd[n - 1] = ptx::addc(odd[n - 1], 0, carry);
    }

    static inline void mad_row_msb(
        device uint32_t* odd,
        device uint32_t* even,
        device const uint32_t* a,
        uint32_t bi,
        size_t n = TLC,
    ...
    static inline void
    multiply_raw_device(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
...
#pragma unroll
    ...
    static inline void
    ingo_multiply_raw_device(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
        device const uint32_t* a = as.limbs;
        device const uint32_t* b = bs.limbs;
        uint32_t carry = 0;
        ...
        for (i = 1; i < TLC - 1; i += 2) {
            ...
            even[i + 1] = ptx::mad_lo_cc(a[1], b[i], even[i + 1], carry);
            even[i + 2] = ptx::madc_hi_cc(a[1], b[i], even[i + 2], carry);
            // for loop carry in carry out
            for (j = 3; j < TLC; j += 2) {
            even[i + j] = ptx::madc_lo_cc(a[j], b[i], even[i + j], carry);
            even[i + j + 1] = ptx::madc_hi_cc(a[j], b[i], even[i + j + 1], carry);
            }

            // even bi's
            // multiply accumulate even part of new row with even part of prev row // needs a carry
            // j = 0, no carry in, only carry out
            even[i + 1] = ptx::mad_lo_cc(a[0], b[i + 1], even[i + 1], carry);
            even[i + 2] = ptx::madc_hi_cc(a[0], b[i + 1], even[i + 2], carry);
            // for loop, carry in, carry out.
            for (j = 2; j < TLC; j += 2) {
            even[i + j + 1] = ptx::madc_lo_cc(a[j], b[i + 1], even[i + j + 1], carry);
            even[i + j + 2] = ptx::madc_hi_cc(a[j], b[i + 1], even[i + j + 2], carry);
            }
            even[i + j + 1] = ptx::addc(even[i + j + 1], 0, carry); // handling last carry
        ...
        even[i + 1] = ptx::addc(even[i + 1], 0, carry);
    }
...
#pragma unroll
            ...
            // multiply accumulate odd part of new row with even part prev row (doesnt need a carry)
            // j = 1, no carry in, only carry out
            even[i + first_active_j_odd] = ptx::mad_lo_cc(a[first_active_j_odd], b[i], even[i + first_active_j_odd], carry);
            even[i + first_active_j_odd + 1] =
            ptx::madc_hi_cc(a[first_active_j_odd], b[i], even[i + first_active_j_odd + 1], carry);
            // for loop carry in carry out
#pragma unroll
            ...
            ptx::mad_lo_cc(a[first_active_j_even1], b[i + 1], even[first_active_j_even1 + i + 1], carry);
            even[first_active_j_even1 + i + 2] =
            ptx::madc_hi_cc(a[first_active_j_even1], b[i + 1], even[first_active_j_even1 + i + 2], carry);
        // for loop, carry in, carry out.
#pragma unroll
...
#pragma unroll
        for (i = 1; i < 2 * TLC - 2; i++)
            even[i + 1] = ptx::addc_cc(even[i + 1], odd[i], carry);
        even[i + 1] = ptx::addc(even[i + 1], 0, carry);
    }

    static inline void
    multiply_lsb_raw_device(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
...
#pragma unroll
    ...
    static inline void
    multiply_msb_raw_device(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
        device const uint32_t* a = as.limbs;
        device const uint32_t* b = bs.limbs;
        device uint32_t* even = rs.limbs;
...
#pragma unroll
    ...
    static inline void
    multiply_raw_host(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
        device const uint32_t* a = as.limbs;
        device const uint32_t* b = bs.limbs;
        device uint32_t* r = rs.limbs;
    ...
    static inline void
    multiply_raw_lsb(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
        return multiply_raw_host(as, bs, rs);
    }

    static inline void
    multiply_raw_msb(device const ff_storage& as, device const ff_storage& bs, device ff_wide_storage& rs)
    {
        return multiply_raw_host(as, bs, rs);
    }
...
public:
    ...
    {
        uint32_t carry = 0;
        r[0] = ptx::sub_cc(x[0], y[0], carry);
#pragma unroll
        for (unsigned i = 1; i < num_limbs; i++)
            r[i] = ptx::subc_cc(x[i], y[i], carry);
        return ptx::subc(0, 0, carry);
    }
